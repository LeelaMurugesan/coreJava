...
public class Validator {
	...
	public static boolean checkDateFormat(String newDate) throws InvalidInputDataException {
        boolean result = false;
        int len = newDate.length();
        String[] data1 = newDate.split("/");
        int date = Integer.parseInt(data1[0]);
        int mon =Integer.parseInt(data1[1]);
        int yrs =Integer.parseInt(data1[2]);
        for (int index = 0; index < len; index++) {
            int asc = (int) newDate.charAt(index);
            if ((asc < 46)|| (asc > 58) ) 
	...
	public static boolean Checkfees(float fees) throws InvalidInputDataException {
        boolean result = false;
        if (fees > 0) {
            result = true;
        } else
